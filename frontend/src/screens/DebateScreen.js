 ...
 *
 * 핵심 변경:
 * - TTS가 끝난 후 서버에 "ready" 신호 전송
 * - 서버는 ready 신호를 받아야 다음 발언 생성
 * - 이렇게 해야 화면과 TTS 속도가 일치
 */
...
const parseSegments = (text) => {
  ...
  lines.forEach(line => {
    ...
    } else { segments.push({ type: 'text', content: line }); }
  });
...
const speakAndWaitSafe = (text, options) => new Promise((resolve) => {
  ...
  const timeout = setTimeout(finish, 90000); // 긴 발언 대비 90초
  Speech.speak(text, {
    ...options,
...
const getVoiceSettings = async (memberId) => {
  ...
  try {
    ...
    const korean = available.filter(v =>
      v.language?.startsWith('ko') || v.identifier?.toLowerCase().includes('kr')
    );
    if (korean.length > 0) {
...
const DebateScreen = ({ issue, duration = 40, onFinish }) => {
  ...
  const [roundInfo,  setRoundInfo]  = useState("");   // 라운드 표시
  ...
  const processTtsQueue = useCallback(async () => {
    ...
    while (ttsQueue.current.length > 0) {
      if (!ttsEnabledRef.current) {
        // TTS 꺼진 경우 큐 전부 처리(스킵)하고 ready 신호 전송
        while (ttsQueue.current.length > 0) {
  ...
  useEffect(() => {
    ...
    ws.onerror = () => {
      ...
      Alert.alert("연결 실패", `Railway 서버 주소를 확인하세요.\n${BACKEND_WS_URL}`);
      setIsFinished(true);
    };
  ...
  const downloadDebateLog = async () => {
    ...
    try {
      ...
      if (!baseDir) { Alert.alert("오류", "저장 경로를 찾을 수 없습니다."); return; }
      const fileUri = baseDir + fileName;
      await FileSystem.writeAsStringAsync(fileUri, logText, { encoding: FileSystem.EncodingType.UTF8 });
      const canShare = await Sharing.isAvailableAsync();
      if (canShare) {
        await Sharing.shareAsync(fileUri, { mimeType:'text/plain', dialogTitle:'AI 의회 토론 기록', UTI:'public.plain-text' });
      } else {
